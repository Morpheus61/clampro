...
export default function Dashboard({ onAdminClick }: DashboardProps) {
  ...
  const { data: packagingData, isLoading: packageLoading } = usePackagingData();
  ...
  const totalPackaged = packagingData.reduce((acc, curr) => acc + (curr.weight || 0), 0);
      ...
      <div className="space-y-4">
        {recentDeliveries.map((delivery) => (
          <div key={delivery.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
            <div>
              <p className="font-medium">Lot #{delivery.lotNumber}</p>
              <p className="text-sm text-gray-600">{delivery.weight} kg from {delivery.supplierName}</p>
            </div>
